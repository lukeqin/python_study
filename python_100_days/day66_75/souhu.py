        ...
        for a_tag in soup.body.select('a[href]'):
            ...
            if scheme != 'javascript' and netloc == domain:
                ...
                if not redis_client.sismember('visited_urls', full_url):
                    redis_client.rpush('m_souhu_task', full_url)
...
class SpiderThread(Thread):
    ...
    def run(self):
        redis_client = redis.Redis(host='192.168.59.130', port=6379, password='12345678')
        mongo_client = pymongo.MongoClient(host='192.168.59.130', port=27017)
        thread_local.redis_client = redis_client
        thread_local.mongo_client = mongo_client.msohu
        while True:
                ...
                if html_page not in [None, '']:
                    ...
                    if not sohu_data_coll.find_one({'_id': doc_id}):
                        sohu_data_coll.insert_one(
                            {'_id': doc_id, 'url': current_url, 'page': Binary(
                                zlib.compress(pickle.dumps(html_page)))})
